...
pacman::p_load(optparse, tidyverse, plyr, RColorBrewer, extrafont, ggforce, ggstar, RcppCNPy, reshape2, lemon, plotly,
...
# Loads data ~
data <- as.matrix(read.table("NLSparrow_MinInd90_SNPs.cov"))
annot <- read.table("NLSparrow.labels", sep = "\t", header = FALSE, stringsAsFactors = FALSE)

#MissingData <- read.table("BSG_AtlanticCod--AllSamples.GL-MissingData.txt", sep = "\t", header = FALSE)
...
PCA_Annot$Population <- ifelse(grepl("8L", PCA_Annot$Sample_ID), "Northern Norway",
                        ifelse(grepl("8M", PCA_Annot$Sample_ID), "Northern Norway",
                        ...
                        ifelse(grepl("PI22NLD0001M", PCA_Annot$Sample_ID), "Target",
                        ifelse(grepl("PD22NLD0146F", PCA_Annot$Sample_ID), "Target´s Companion",
                        ifelse(grepl("PD22NLD0147F", PCA_Annot$Sample_ID), "Target´s Companion", "Error")))))))))
...
PCA_Annot$Species <- ifelse(PCA_Annot$Population %in% c("Northern Norway", "Sales", "Target´s Companion"), "House",
                     ifelse(PCA_Annot$Population %in% c("Chokpak", "Lesina"), "Spanish",
                     ifelse(PCA_Annot$Population %in% c("Guglionesi"), "Italian",
                     ifelse(PCA_Annot$Population %in% c("Target"), "Uncertain", "Error"))))
...
MyLegend_Plot <-
  ...
  #scale_fill_manual(values = c("#9e0142", "#d53e4f", "#f46d43", "#fdae61", "#fee08b", "#e6f598",
  #"#abdda4", "#66c2a5", "#3288bd", "#5e4fa2", "#969696")) +
  ...
  guides(starshape = guide_legend(title = "Species", title.theme = element_text(size = 16, face = "bold"),
                                  label.theme = element_text(size = 15),
                                  override.aes = list(starshape = Shapes, size = 5, starstroke = .15), nrow = 1, order = 1),
         fill = guide_legend(title = "Population", title.theme = element_text(size = 16, face = "bold"),
                             label.theme = element_text(size = 15),
                             override.aes = list(starshape = 21, size = 5, starstroke = .15), nrow = 1, order = 2),
         colour = "none")
