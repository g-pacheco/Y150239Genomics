...
PCA_Annot$Population <- ifelse(grepl("8L", PCA_Annot$Sample_ID), "Helgeland",
                        ...
                        ifelse(grepl("PD22NLD0147F", PCA_Annot$Sample_ID), "Garderen", "Error")))))))))
...
PCA_Annot$Species <- ifelse(PCA_Annot$Population %in% c("Helgeland", "Sales", "Garderen"), "House",
...
# Defines the shapes to be used for each Group ~
Shapes_2 <- as.vector(c(15, 25, 4, 1))
...
PCA_12_Blog <-
  ...
  geom_mark_ellipse(aes(filter = Sample_ID == "PI22NLD0001M", label = "Italian-looking House Sparrow"),
                    label.fill = "#d9d9d9", expand = unit(2.5, "mm"), con.border = "one", label.family = ".SF Compact Rounded",
