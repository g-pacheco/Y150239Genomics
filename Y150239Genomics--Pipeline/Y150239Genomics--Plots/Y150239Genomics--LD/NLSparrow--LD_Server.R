### The BEGINNING ~~~~~
##
# ~ Plots BSG_Combined--LD | First written by Filipe G. Vieira & Emma Fox with later slight modifications by George Pacheco.
...
opt$ld_files = "BSG_Lumpfish--AllSamples--SNPs_Ind50.LD_Out.list"
opt$plot_x_lim = 100
opt$fit_level = 100
opt$plot_size = "2,5"
opt$max_kb_dist = 100
...
# Corrects LD State ~
levels(fit_data$File <- sub("BSG_Lumpfish--AllSamples_Ind30_SNPs.Pruned.LD.gz", "Weight 0.05", fit_data$File))
levels(fit_data$File <- sub("BSG_Lumpfish--AllSamples_Ind30_SNPs.Weight2.Pruned.LD.gz", "Weight 0.2", fit_data$File))
levels(fit_data$File <- sub("BSG_Lumpfish--AllSamples_Ind30_SNPs.LD.gz", "No Pruning", fit_data$File))
levels(ld_data$File <- sub("BSG_Lumpfish--AllSamples_Ind30_SNPs.Pruned.LD.gz", "Weight 0.05", ld_data$File))
levels(ld_data$File <- sub("BSG_Lumpfish--AllSamples_Ind30_SNPs.Weight2.Pruned.LD.gz", "Weight 0.2", ld_data$File))
levels(ld_data$File <- sub("BSG_Lumpfish--AllSamples_Ind30_SNPs.LD.gz", "No Pruning", ld_data$File))


# Reorders LD State ~
fit_data$File <- factor(fit_data$File, ordered = T,
                        levels = c("No Pruning", "Weight 0.2", "Weight 0.05"))
...
# Saves plot ~
ggsave(plot = plot, file = "Layka_Median_All.pdf",
